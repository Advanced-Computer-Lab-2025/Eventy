...
export class ApplicationController {
  ...
  async applyToBooth(req, res, _next) {
    try {
      const vendorId = req.user._id;
  ...
  async getAttendeeByToken(req, res, next) {
    try {
      ...
      console.log(
        `🟢 Backend: Token received (first 20 chars): ${token?.substring(0, 20)}...`
      );
      ...
      if (!application) {
        console.error(
          "🔴 Backend: Application not found for ID:",
      ...
      if (!attendee) {
        console.error(
          "🔴 Backend: Attendee not found for email:",
          decoded.attendeeEmail
        );
        console.log(
          "🟢 Backend: Available attendees:",
          application.attendees.map((a) => a.email)
        );
        console.log("🟢 Backend: ========================================");
        return res.status(404).json({
          success: false,
      ...
      const location =
        application.event?.location || application.locationPreference || "N/A";
